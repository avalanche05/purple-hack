...
class DatasetBoostrapping:
    def __init__(self, data_lists) -> None:
        ...
        self.dependencies_id = list(set(r["id"] for r in self.original["dependencies"]))
    ...
    def perturb(self, seed, num_resources=6, num_tasks=9, num_dependencies=6):
        ...
        for _ in range(num_resources):
            output["resources"].append({
                'id': random.choice(self.resourses_id),
        ...
        for _ in range(num_tasks):
            ...
            output["tasks"].append({
                'id': random.choice(self.tasks_id),
                'effort': random.choice(self.tasks_effort),
                'project_role_id': random.choice(self.tasks_role)
            })
        
        for _ in range(num_dependencies):
            from_task = random.choice(self.tasks_id)
            # idx = tasks_id.index(from_task)
            to_task = random.choice([id_ for id_ in self.tasks_id if id_ != from_task])

            output["dependencies"].append({
                'id': random.choice(self.dependencies_id),
                'from': from_task,
                'to': to_task
            })
